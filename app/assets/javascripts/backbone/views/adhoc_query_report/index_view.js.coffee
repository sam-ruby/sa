...
This view main renders the form for adhoc query_report. This view triggers the 
behavior of two sub views 1) query_comparison as advanced mode and 
2) search_view When click on the toggle button, it would swicth the 
behavior for the two subview
###
...
class Searchad.Views.AdhocQuery.IndexView extends Backbone.View
  initialize: (options) ->
    ...
    # the available_end_date is by default is the max_date(most recent date) 
    # availabe, which is defined _main_content.html.haml
    @available_end_date = Max_date
    @default_week_apart = 2
    @data
    # by default, turn on query_comparison. if it is false, it means on 
    # adhoc search mode
  ...
  # this toggle_search_mode is not only triggerd by click_toogle_search_mode, 
  # it also get triggered by the places where 
  ...
  toggle_search_mode: (query_comparison_on)->
    ...
    if query_comparison_on
      ...
      @query_form.find('span.switch_mode_text').text(
        @switch_query_comparison_text)
  ...
  change_date_picked: ->
    ...
    before_start_date = new Date(
      new Date(query_date) - weeks_apart*7*day_in_ms).toString('MMM, d, yyyy')
    before_end_date = new Date(
      new Date(query_date) - day_in_ms).toString('MMM, d, yyyy')
    after_start_date = query_date .toString('MMM, d, yyyy')
    after_end_date = new Date(new Date(query_date) - (
      -(weeks_apart*7-1)*day_in_ms)).toString('MMM, d, yyyy')
    
    $('.date_range_display').html(
      '<p class= "selected_date_range_text">Selected <span>'+
      before_start_date+' to '+ before_end_date +
      ' as before</span> and <span>' + after_start_date + ' to ' +
      after_end_date + '</span> as after</p>')

  # when the selected window changes, this function is needed to change 
  # thw availabe date on the datepicker input
  change_select: ->
    weeks_apart= @query_form.find('select').val()
    query_date= @query_form.find('input.datepicker').datepicker('getDate')
  ...
  search: =>
    data =
      weeks_apart: @query_form.find('select').val()
      query_date:@query_form.find('input.datepicker').datepicker(
        'getDate').toString('M-d-yyyy')
