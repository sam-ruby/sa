class QueryPerformance < BaseModel
  ...
  def self.get_comp_analysis(
    ...
    select_cols = %q{distinct query_str as query, assort_overlap_indexed as catalog_overlap,
    ...
    if query.nil? || query.empty?
      ...
      select(select_cols).where(%q{week=? and year=? and assort_overlap_indexed > 0.5
        and show_rate<0.5 and rel_score is not null and query_str = ?},
        week, year, query).order(order_str).page(page).per(limit)
    elsif fuzzy
      select(select_cols).where(%q{week=? and year=? and assort_overlap_indexed > 0.5
        and show_rate<0.5 and rel_score is not null and query_str like ?},
        week, year, "%#{query}%").order(order_str).page(page).per(limit)
    end
  end
